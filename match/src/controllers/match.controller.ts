...
export const sendCreature = async (request: SendCreatureRequest, reply: FastifyReply) => {
  try {
    ...
    if (match.state != 'in_progress') {
      reply.code(400).send({ message: 'You cannot send creature while match is ' + match.state })
    }
    if (match.rounds.length == 0) {
      if (username == match.player1Username) {
        const newRound = await Round.create({matchId, roundNumber: 1, creature1Id: creatureName})
        ...
        const newRound = await Round.create({matchId, roundNumber: 1, creature2Id: creatureName})
        match.rounds.push(newRound._id)
        await Match.findByIdAndUpdate(matchId, match)
        reply.code(200).send({ message: 'Creature sent successfully, round was played', newRound })
      }
      ...
      if (lastRound.winner) {
        if (username == match.player1Username) {
          const newRound = await Round.create({matchId, roundNumber: match.rounds.length+1, creature1Id: creatureName})
          match.rounds.push(newRound._id)
          await Match.findByIdAndUpdate(matchId, match)
          reply.code(200).send({ message: 'Creature sent successfully, round was played', newRound })
        } else {
          const newRound = await Round.create({matchId, roundNumber: match.rounds.length+1, creature2Id: creatureName})
          match.rounds.push(newRound._id)
          await Match.findByIdAndUpdate(matchId, match)
          reply.code(200).send({ message: 'Creature sent successfully, round was played', newRound })
        }
        ...
        if (match.rounds.length == 5) {
          ...
          const creditsReq = await axios.post(`http://${USER_SERVICE_URL}/api/user/add/credits`, {
            credit: 100,
